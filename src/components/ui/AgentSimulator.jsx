...
const RUNS = {
  math: {
    ...
    logs: [
      ...
      { text: "➔ Router Node: Checking math query trigger keywords ('parameters', 'dimension', 'head')...", delay: 700 },
      { text: "➔ ROUTE ➔ Redirecting to Python Sandbox Node (skip LLM generation to prevent math hallucination)...", delay: 1200 },
      { text: "➔ Python Sandbox Node: Evaluating expression: '12 * (hidden_dim * hidden_dim * 4)'...", delay: 1700 },
      { text: "➔ Python Sandbox Node: Execution successful. Calculated exact parameter count: 50,331,648.", delay: 2200 },
      { text: "➔ LLM Judge Node: Validating results safety and output format...", delay: 2700 },
      { text: "➔ System Output: 'A single 16-head Transformer block with a hidden dimension of 1024 contains exactly 50,331,648 learnable weights (excluding biases).'", delay: 3200 }
    ],
    status: 'completed'
  },
  rag: {
    ...
    logs: [
      ...
      { text: "➔ LLM Judge Node: Faithfulness score is 0.94 (THRESHOLD: >= 0.70). Gate status: APPROVED.", delay: 2700 },
      { text: "➔ System Output: 'Query represents the current search word, Keys represent all other words in the sequence, and Values are the vector content returned when a Query matches a Key.'", delay: 3200 }
    ],
    status: 'completed'
  },
  malicious: {
    ...
    logs: [
      ...
      { text: "➔ System Output: 'Blocked: I cannot generate scripts or code designed to destroy file system structures. Query score: 0.00 (Unsafe).'", delay: 3200 }
    ],
    status: 'blocked'
  }
}
  ...
  return (
    <div className="w-full p-6 rounded-lg border font-mono text-xs text-left" style={{ background: 'var(--bg-secondary)', borderColor: 'var(--border)' }}>
      ...
      <p className="font-sans text-sm mb-6 leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
        Select a query type to test how the agent's conditional nodes dynamically route the prompt and guard against hallucinations.
      </p>
      ...
      >
        <div className="space-y-2">
          {activeRun && (
            <p className="font-bold border-b pb-2 mb-3" style={{ borderColor: 'var(--border)', color: 'var(--text-primary)' }}>
